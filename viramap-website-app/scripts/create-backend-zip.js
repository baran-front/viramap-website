...
function createBackendZip() {
  ...
  if (!fs.existsSync(nextDir)) {
    logError('پوشه .next یافت نشد! لطفاً ابتدا بیلد بگیرید: pnpm build:webpack');
    process.exit(1);
  }
  ...
  try {
    ...
    if (isWindows) {
      ...
      const itemsArray = existingItems.map(item => {
        const itemPath = path.join(rootDir, item).replace(/\\/g, '\\\\').replace(/'/g, "''");
        return `'${itemPath}'`;
      });
      ...
      filteredItems.forEach(item => {
        if (item === '.next') {
          ...
          if (fs.existsSync(nextStandalone)) finalItems.push('.next/standalone');
          if (fs.existsSync(nextStatic)) finalItems.push('.next/static');
          if (fs.existsSync(nextServer)) finalItems.push('.next/server');
          // فایل‌های root .next
          const nextFiles = ['BUILD_ID', 'package.json', 'required-server-files.json'];
